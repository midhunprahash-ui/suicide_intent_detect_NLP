...
# Must be identical to the ones used during the initial fine-tuning.
...
if __name__ == '__main__':
    ...
    try:
        ...
        age_scaler = joblib.load('age_scaler.bin')
    except FileNotFoundError:
        print("\nERROR: 'age_scaler.bin' not found. Run script #1 first.")
        exit()
    ...
    print("\n--- Loading the pruned model for fine-tuning ---")
    model = MultimodalSuicideRiskModel().to(DEVICE)
    try:
        model.load_state_dict(torch.load('pruned_only_model.bin', map_location=DEVICE))
    except FileNotFoundError:
        print("\nERROR: 'pruned_only_model.bin' not found. Run script #2 first.")
        exit()
    ...
    for epoch in range(3): # 3 epochs is usually enough
        print(f'Fine-Tuning Epoch {epoch + 1}/3')
        train_loss = train_epoch(model, train_loader, loss_fn, optimizer, DEVICE)
        val_loss = eval_model(model, val_loader, loss_fn, DEVICE)
        print(f'  Train loss: {train_loss:.4f}, Validation loss: {val_loss:.4f}')
        if val_loss < best_val_loss:
            best_val_loss = val_loss
            torch.save(model.state_dict(), 'pruned_and_finetuned_model.bin')
            print(f"  New best model saved to 'pruned_and_finetuned_model.bin'")
    print("\nFine-tuning of pruned model complete.")
